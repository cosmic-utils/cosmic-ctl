...
fn test_list_command() {
    ...
    assert!(stdout.contains(COSMIC_COMP));
    assert!(stdout.contains(ENTRY_AUTOTILE));
    assert!(stdout.contains(ENTRY_AUTOTILE_BEHAVIOR));
    assert!(stdout.contains(&format!("v{}", VERSION_1)));
    assert!(stdout.contains(&format!("v{}", VERSION_2)));
    assert!(stdout.contains(VALUE_TRUE));
    assert!(stdout.contains(VALUE_PER_WORKSPACE));
    assert!(stdout.contains("Total entries: 3"));
}
...
fn test_list_command_verbose() {
    ...
    assert!(stdout.contains(&format!("Found: {}/v1/{}", COSMIC_COMP, ENTRY_AUTOTILE)));
    assert!(stdout.contains(COSMIC_COMP));
    assert!(stdout.contains(ENTRY_AUTOTILE));
    assert!(stdout.contains(&format!("v{}", VERSION_1)));
    assert!(stdout.contains(VALUE_TRUE));
    assert!(stdout.contains("Total entries: 1"));
}
